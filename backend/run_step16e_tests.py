...
for q_label, query_text in questions:
    ...
    if "**Qualitative Visual Interpretation (VLM):**" in answer_text:
        ...
        displayed_interpretation = part

    # Find raw VLM answer in evidence if present
    raw_vlm_output = displayed_interpretation
    for ev in data.get("evidence", []):
        if "(raw:" in ev:
            # extract raw
            raw_part = ev.split("(raw:")[1].rstrip(").").strip()
            if raw_part.startswith("'") and raw_part.endswith("'"):
                raw_vlm_output = raw_part[1:-1]
            elif raw_part.startswith('"') and raw_part.endswith('"'):
                raw_vlm_output = raw_part[1:-1]
            else:
                raw_vlm_output = raw_part
    ...
    res_entry = {
        ...
        "displayed_interpretation": displayed_interpretation,
    ...
    print(f"  Raw VLM Output: {raw_vlm_output}")
    print(f"  Validation Status: {vlm_val_status}")
    print(f"  Final Displayed Interpretation: {displayed_interpretation}")
    print(f"  Detector Changed %: {changed_pct}%")
    print(f"  Execution Mode: {data.get('executionMode')}")
    print(f"  is_mock: {data.get('isMock')}")
    print(f"  Confidence: {data.get('confidence')}")

# Save full results
with open(r"C:\Users\Lenovo\Desktop\SatQuery-AI\backend\step16e_audit_results.json", "w") as f:
    json.dump(results, f, indent=2)

print("\nAll 3 questions tested successfully! Results saved to step16e_audit_results.json")
